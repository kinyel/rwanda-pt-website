/**
 * Country calling codes for the international phone field.
 *
 * WHY THIS IS HAND-BUILT RATHER THAN A LIBRARY. The obvious choice is
 * libphonenumber-js, and on a different site it would be the right one. Its
 * smallest metadata bundle is roughly 75KB before the component that reads it,
 * which is more JavaScript than the entire contact page currently ships. The
 * README for the hero backdrop already records that a large share of this
 * site's audience is on 3G; spending that budget so a phone number can be
 * reformatted as it is typed is a bad trade.
 *
 * What is here instead is the list itself, which is small, plus a length range
 * per country for the markets PrimeTrack actually serves. Everything else falls
 * back to the E.164 limits, which is the same answer a library would give for a
 * number it has no specific rule for.
 *
 * Flags are NOT stored. A flag emoji is the country's two ISO letters shifted
 * into the regional-indicator block, so it is derived in `flagFor` below and
 * costs nothing to keep.
 *
 * `len` is the national significant number, so the digits AFTER the dial code:
 * a Rwandan mobile is 788123456, nine digits, dialled as +250 788123456. It is
 * given only where it is known to be exact; leaving it off is not an omission,
 * it is the honest answer.
 */
export interface Country {
  /** ISO 3166-1 alpha-2. Also the key for the flag emoji. */
  iso: string;
  name: string;
  /** E.164 country calling code, without the plus. */
  dial: string;
  /** [min, max] digits after the dial code. Omitted where not certain. */
  len?: [number, number];
}

/** E.164 allows at most 15 digits in total, dial code included. */
export const E164_MAX = 15;

/**
 * The two ISO letters map onto U+1F1E6..U+1F1FF, so "RW" becomes 🇷🇼 with no
 * image, no sprite sheet and no request.
 */
export function flagFor(iso: string): string {
  return String.fromCodePoint(...[...iso.toUpperCase()].map((c) => 0x1f1e6 + c.charCodeAt(0) - 65));
}

/** Where the field starts. This is the Rwanda site. */
export const DEFAULT_ISO = 'RW';

/**
 * Shown above the divider, because between them they cover almost every number
 * this form will ever receive: Rwanda and its neighbours, plus the two other
 * countries PrimeTrack operates in.
 */
export const PRIORITY_ISO = ['RW', 'KE', 'UG', 'TZ', 'BI', 'CD', 'NG', 'EG'];

export const countries: Country[] = [
  { iso: 'AF', name: 'Afghanistan', dial: '93' },
  { iso: 'AL', name: 'Albania', dial: '355' },
  { iso: 'DZ', name: 'Algeria', dial: '213' },
  { iso: 'AS', name: 'American Samoa', dial: '1684' },
  { iso: 'AD', name: 'Andorra', dial: '376' },
  { iso: 'AO', name: 'Angola', dial: '244', len: [9, 9] },
  { iso: 'AI', name: 'Anguilla', dial: '1264' },
  { iso: 'AG', name: 'Antigua and Barbuda', dial: '1268' },
  { iso: 'AR', name: 'Argentina', dial: '54' },
  { iso: 'AM', name: 'Armenia', dial: '374' },
  { iso: 'AW', name: 'Aruba', dial: '297' },
  { iso: 'AU', name: 'Australia', dial: '61', len: [9, 9] },
  { iso: 'AT', name: 'Austria', dial: '43' },
  { iso: 'AZ', name: 'Azerbaijan', dial: '994' },
  { iso: 'BS', name: 'Bahamas', dial: '1242' },
  { iso: 'BH', name: 'Bahrain', dial: '973', len: [8, 8] },
  { iso: 'BD', name: 'Bangladesh', dial: '880' },
  { iso: 'BB', name: 'Barbados', dial: '1246' },
  { iso: 'BY', name: 'Belarus', dial: '375' },
  { iso: 'BE', name: 'Belgium', dial: '32' },
  { iso: 'BZ', name: 'Belize', dial: '501' },
  { iso: 'BJ', name: 'Benin', dial: '229' },
  { iso: 'BM', name: 'Bermuda', dial: '1441' },
  { iso: 'BT', name: 'Bhutan', dial: '975' },
  { iso: 'BO', name: 'Bolivia', dial: '591' },
  { iso: 'BA', name: 'Bosnia and Herzegovina', dial: '387' },
  { iso: 'BW', name: 'Botswana', dial: '267' },
  { iso: 'BR', name: 'Brazil', dial: '55' },
  { iso: 'VG', name: 'British Virgin Islands', dial: '1284' },
  { iso: 'BN', name: 'Brunei', dial: '673' },
  { iso: 'BG', name: 'Bulgaria', dial: '359' },
  { iso: 'BF', name: 'Burkina Faso', dial: '226' },
  { iso: 'BI', name: 'Burundi', dial: '257', len: [8, 8] },
  { iso: 'KH', name: 'Cambodia', dial: '855' },
  { iso: 'CM', name: 'Cameroon', dial: '237', len: [9, 9] },
  { iso: 'CA', name: 'Canada', dial: '1', len: [10, 10] },
  { iso: 'CV', name: 'Cape Verde', dial: '238' },
  { iso: 'KY', name: 'Cayman Islands', dial: '1345' },
  { iso: 'CF', name: 'Central African Republic', dial: '236' },
  { iso: 'TD', name: 'Chad', dial: '235' },
  { iso: 'CL', name: 'Chile', dial: '56' },
  { iso: 'CN', name: 'China', dial: '86' },
  { iso: 'CO', name: 'Colombia', dial: '57' },
  { iso: 'KM', name: 'Comoros', dial: '269' },
  { iso: 'CG', name: 'Congo-Brazzaville', dial: '242' },
  { iso: 'CD', name: 'Congo-Kinshasa', dial: '243', len: [9, 9] },
  { iso: 'CK', name: 'Cook Islands', dial: '682' },
  { iso: 'CR', name: 'Costa Rica', dial: '506' },
  { iso: 'CI', name: "Côte d'Ivoire", dial: '225' },
  { iso: 'HR', name: 'Croatia', dial: '385' },
  { iso: 'CU', name: 'Cuba', dial: '53' },
  { iso: 'CW', name: 'Curaçao', dial: '599' },
  { iso: 'CY', name: 'Cyprus', dial: '357' },
  { iso: 'CZ', name: 'Czechia', dial: '420', len: [9, 9] },
  { iso: 'DK', name: 'Denmark', dial: '45', len: [8, 8] },
  { iso: 'DJ', name: 'Djibouti', dial: '253' },
  { iso: 'DM', name: 'Dominica', dial: '1767' },
  { iso: 'DO', name: 'Dominican Republic', dial: '1809' },
  { iso: 'EC', name: 'Ecuador', dial: '593' },
  { iso: 'EG', name: 'Egypt', dial: '20', len: [10, 10] },
  { iso: 'SV', name: 'El Salvador', dial: '503' },
  { iso: 'GQ', name: 'Equatorial Guinea', dial: '240' },
  { iso: 'ER', name: 'Eritrea', dial: '291' },
  { iso: 'EE', name: 'Estonia', dial: '372' },
  { iso: 'SZ', name: 'Eswatini', dial: '268' },
  { iso: 'ET', name: 'Ethiopia', dial: '251', len: [9, 9] },
  { iso: 'FJ', name: 'Fiji', dial: '679' },
  { iso: 'FI', name: 'Finland', dial: '358' },
  { iso: 'FR', name: 'France', dial: '33', len: [9, 9] },
  { iso: 'GA', name: 'Gabon', dial: '241' },
  { iso: 'GM', name: 'Gambia', dial: '220' },
  { iso: 'GE', name: 'Georgia', dial: '995' },
  { iso: 'DE', name: 'Germany', dial: '49' },
  { iso: 'GH', name: 'Ghana', dial: '233', len: [9, 9] },
  { iso: 'GI', name: 'Gibraltar', dial: '350' },
  { iso: 'GR', name: 'Greece', dial: '30', len: [10, 10] },
  { iso: 'GL', name: 'Greenland', dial: '299' },
  { iso: 'GD', name: 'Grenada', dial: '1473' },
  { iso: 'GU', name: 'Guam', dial: '1671' },
  { iso: 'GT', name: 'Guatemala', dial: '502' },
  { iso: 'GN', name: 'Guinea', dial: '224' },
  { iso: 'GW', name: 'Guinea-Bissau', dial: '245' },
  { iso: 'GY', name: 'Guyana', dial: '592' },
  { iso: 'HT', name: 'Haiti', dial: '509' },
  { iso: 'HN', name: 'Honduras', dial: '504' },
  { iso: 'HK', name: 'Hong Kong', dial: '852', len: [8, 8] },
  { iso: 'HU', name: 'Hungary', dial: '36' },
  { iso: 'IS', name: 'Iceland', dial: '354' },
  { iso: 'IN', name: 'India', dial: '91', len: [10, 10] },
  { iso: 'ID', name: 'Indonesia', dial: '62' },
  { iso: 'IR', name: 'Iran', dial: '98' },
  { iso: 'IQ', name: 'Iraq', dial: '964' },
  { iso: 'IE', name: 'Ireland', dial: '353' },
  { iso: 'IL', name: 'Israel', dial: '972' },
  { iso: 'IT', name: 'Italy', dial: '39' },
  { iso: 'JM', name: 'Jamaica', dial: '1876' },
  { iso: 'JP', name: 'Japan', dial: '81' },
  { iso: 'JO', name: 'Jordan', dial: '962' },
  { iso: 'KZ', name: 'Kazakhstan', dial: '7' },
  { iso: 'KE', name: 'Kenya', dial: '254', len: [9, 9] },
  { iso: 'KI', name: 'Kiribati', dial: '686' },
  { iso: 'XK', name: 'Kosovo', dial: '383' },
  { iso: 'KW', name: 'Kuwait', dial: '965', len: [8, 8] },
  { iso: 'KG', name: 'Kyrgyzstan', dial: '996' },
  { iso: 'LA', name: 'Laos', dial: '856' },
  { iso: 'LV', name: 'Latvia', dial: '371' },
  { iso: 'LB', name: 'Lebanon', dial: '961' },
  { iso: 'LS', name: 'Lesotho', dial: '266' },
  { iso: 'LR', name: 'Liberia', dial: '231' },
  { iso: 'LY', name: 'Libya', dial: '218' },
  { iso: 'LI', name: 'Liechtenstein', dial: '423' },
  { iso: 'LT', name: 'Lithuania', dial: '370' },
  { iso: 'LU', name: 'Luxembourg', dial: '352' },
  { iso: 'MO', name: 'Macao', dial: '853' },
  { iso: 'MG', name: 'Madagascar', dial: '261' },
  { iso: 'MW', name: 'Malawi', dial: '265' },
  { iso: 'MY', name: 'Malaysia', dial: '60' },
  { iso: 'MV', name: 'Maldives', dial: '960' },
  { iso: 'ML', name: 'Mali', dial: '223' },
  { iso: 'MT', name: 'Malta', dial: '356' },
  { iso: 'MH', name: 'Marshall Islands', dial: '692' },
  { iso: 'MR', name: 'Mauritania', dial: '222' },
  { iso: 'MU', name: 'Mauritius', dial: '230' },
  { iso: 'MX', name: 'Mexico', dial: '52', len: [10, 10] },
  { iso: 'FM', name: 'Micronesia', dial: '691' },
  { iso: 'MD', name: 'Moldova', dial: '373' },
  { iso: 'MC', name: 'Monaco', dial: '377' },
  { iso: 'MN', name: 'Mongolia', dial: '976' },
  { iso: 'ME', name: 'Montenegro', dial: '382' },
  { iso: 'MS', name: 'Montserrat', dial: '1664' },
  { iso: 'MA', name: 'Morocco', dial: '212', len: [9, 9] },
  { iso: 'MZ', name: 'Mozambique', dial: '258' },
  { iso: 'MM', name: 'Myanmar', dial: '95' },
  { iso: 'NA', name: 'Namibia', dial: '264' },
  { iso: 'NR', name: 'Nauru', dial: '674' },
  { iso: 'NP', name: 'Nepal', dial: '977' },
  { iso: 'NL', name: 'Netherlands', dial: '31', len: [9, 9] },
  { iso: 'NC', name: 'New Caledonia', dial: '687' },
  { iso: 'NZ', name: 'New Zealand', dial: '64' },
  { iso: 'NI', name: 'Nicaragua', dial: '505' },
  { iso: 'NE', name: 'Niger', dial: '227' },
  { iso: 'NG', name: 'Nigeria', dial: '234', len: [10, 10] },
  { iso: 'KP', name: 'North Korea', dial: '850' },
  { iso: 'MK', name: 'North Macedonia', dial: '389' },
  { iso: 'NO', name: 'Norway', dial: '47', len: [8, 8] },
  { iso: 'OM', name: 'Oman', dial: '968', len: [8, 8] },
  { iso: 'PK', name: 'Pakistan', dial: '92', len: [10, 10] },
  { iso: 'PW', name: 'Palau', dial: '680' },
  { iso: 'PS', name: 'Palestine', dial: '970' },
  { iso: 'PA', name: 'Panama', dial: '507' },
  { iso: 'PG', name: 'Papua New Guinea', dial: '675' },
  { iso: 'PY', name: 'Paraguay', dial: '595' },
  { iso: 'PE', name: 'Peru', dial: '51', len: [9, 9] },
  { iso: 'PH', name: 'Philippines', dial: '63', len: [10, 10] },
  { iso: 'PL', name: 'Poland', dial: '48', len: [9, 9] },
  { iso: 'PT', name: 'Portugal', dial: '351', len: [9, 9] },
  { iso: 'PR', name: 'Puerto Rico', dial: '1787' },
  { iso: 'QA', name: 'Qatar', dial: '974', len: [8, 8] },
  { iso: 'RO', name: 'Romania', dial: '40', len: [9, 9] },
  { iso: 'RU', name: 'Russia', dial: '7', len: [10, 10] },
  { iso: 'RW', name: 'Rwanda', dial: '250', len: [9, 9] },
  { iso: 'KN', name: 'Saint Kitts and Nevis', dial: '1869' },
  { iso: 'LC', name: 'Saint Lucia', dial: '1758' },
  { iso: 'VC', name: 'Saint Vincent and the Grenadines', dial: '1784' },
  { iso: 'WS', name: 'Samoa', dial: '685' },
  { iso: 'SM', name: 'San Marino', dial: '378' },
  { iso: 'ST', name: 'São Tomé and Príncipe', dial: '239' },
  { iso: 'SA', name: 'Saudi Arabia', dial: '966', len: [9, 9] },
  { iso: 'SN', name: 'Senegal', dial: '221', len: [9, 9] },
  { iso: 'RS', name: 'Serbia', dial: '381' },
  { iso: 'SC', name: 'Seychelles', dial: '248' },
  { iso: 'SL', name: 'Sierra Leone', dial: '232' },
  { iso: 'SG', name: 'Singapore', dial: '65', len: [8, 8] },
  { iso: 'SK', name: 'Slovakia', dial: '421' },
  { iso: 'SI', name: 'Slovenia', dial: '386' },
  { iso: 'SB', name: 'Solomon Islands', dial: '677' },
  { iso: 'SO', name: 'Somalia', dial: '252' },
  { iso: 'ZA', name: 'South Africa', dial: '27', len: [9, 9] },
  { iso: 'KR', name: 'South Korea', dial: '82' },
  { iso: 'SS', name: 'South Sudan', dial: '211', len: [9, 9] },
  { iso: 'ES', name: 'Spain', dial: '34', len: [9, 9] },
  { iso: 'LK', name: 'Sri Lanka', dial: '94' },
  { iso: 'SD', name: 'Sudan', dial: '249' },
  { iso: 'SR', name: 'Suriname', dial: '597' },
  { iso: 'SE', name: 'Sweden', dial: '46' },
  { iso: 'CH', name: 'Switzerland', dial: '41', len: [9, 9] },
  { iso: 'SY', name: 'Syria', dial: '963' },
  { iso: 'TW', name: 'Taiwan', dial: '886' },
  { iso: 'TJ', name: 'Tajikistan', dial: '992' },
  { iso: 'TZ', name: 'Tanzania', dial: '255', len: [9, 9] },
  { iso: 'TH', name: 'Thailand', dial: '66', len: [9, 9] },
  { iso: 'TL', name: 'Timor-Leste', dial: '670' },
  { iso: 'TG', name: 'Togo', dial: '228' },
  { iso: 'TO', name: 'Tonga', dial: '676' },
  { iso: 'TT', name: 'Trinidad and Tobago', dial: '1868' },
  { iso: 'TN', name: 'Tunisia', dial: '216', len: [8, 8] },
  { iso: 'TR', name: 'Türkiye', dial: '90', len: [10, 10] },
  { iso: 'TM', name: 'Turkmenistan', dial: '993' },
  { iso: 'TC', name: 'Turks and Caicos Islands', dial: '1649' },
  { iso: 'TV', name: 'Tuvalu', dial: '688' },
  { iso: 'UG', name: 'Uganda', dial: '256', len: [9, 9] },
  { iso: 'UA', name: 'Ukraine', dial: '380', len: [9, 9] },
  { iso: 'AE', name: 'United Arab Emirates', dial: '971', len: [9, 9] },
  { iso: 'GB', name: 'United Kingdom', dial: '44', len: [10, 10] },
  { iso: 'US', name: 'United States', dial: '1', len: [10, 10] },
  { iso: 'UY', name: 'Uruguay', dial: '598' },
  { iso: 'UZ', name: 'Uzbekistan', dial: '998' },
  { iso: 'VU', name: 'Vanuatu', dial: '678' },
  { iso: 'VE', name: 'Venezuela', dial: '58' },
  { iso: 'VN', name: 'Vietnam', dial: '84', len: [9, 9] },
  { iso: 'YE', name: 'Yemen', dial: '967' },
  { iso: 'ZM', name: 'Zambia', dial: '260', len: [9, 9] },
  { iso: 'ZW', name: 'Zimbabwe', dial: '263' },
];

const byIso = new Map(countries.map((c) => [c.iso, c]));

/**
 * Which country wins when a dial code belongs to more than one.
 *
 * Pasting +1 4155550134 has to resolve to somewhere, and picking the first
 * alphabetical match handed every North American number to Canada. There is no
 * correct answer available from the dial code alone, so this records the more
 * likely one and the visitor can still change it in a tap.
 */
const PREFERRED_FOR_DIAL: Record<string, string> = {
  '1': 'US',
  '7': 'RU',
};

/**
 * The country a full international number belongs to, or undefined.
 * Longest dial code first, so +1264 (Anguilla) beats +1 (United States).
 */
export function countryForDigits(digits: string): Country | undefined {
  const matches = countries
    .filter((c) => digits.startsWith(c.dial))
    .sort((a, b) => b.dial.length - a.dial.length);
  if (matches.length === 0) return undefined;

  const longest = matches[0].dial.length;
  const tied = matches.filter((c) => c.dial.length === longest);
  if (tied.length === 1) return tied[0];

  const preferred = PREFERRED_FOR_DIAL[tied[0].dial];
  return tied.find((c) => c.iso === preferred) ?? tied[0];
}

/** Digits allowed after this country's dial code, within the E.164 ceiling. */
export function nationalMax(c: Country): number {
  return Math.min(c.len?.[1] ?? E164_MAX, E164_MAX - c.dial.length);
}

export function countryFor(iso: string): Country {
  return byIso.get(iso) ?? byIso.get(DEFAULT_ISO)!;
}

/**
 * The list in the order the dropdown shows it: the countries this form
 * actually receives numbers from, then everyone else alphabetically.
 */
export const orderedCountries: Country[] = [
  ...PRIORITY_ISO.map((iso) => byIso.get(iso)!).filter(Boolean),
  ...countries.filter((c) => !PRIORITY_ISO.includes(c.iso)),
];

/** How many entries the priority block holds, so the list can rule under it. */
export const PRIORITY_COUNT = PRIORITY_ISO.filter((iso) => byIso.has(iso)).length;
